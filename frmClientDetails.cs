    ...
    public partial class frmClientDetails : Form
    {
        ...
        public void loadpay()
        {
            try
            {
                ...
                con.Open();
                cmd = new SqlCommand("select RTRIM(ScholarNo)[Student No.],RTRIM(Student_name)[Student Names] from Student order by ID DESC", con);
        ...
        private void textBoxX1_TextChanged(object sender, EventArgs e)
        {
            try
            {
                con = new SqlConnection(cs.DBConn);
                con.Open();
                cmd = new SqlCommand("select RTRIM(ScholarNo)[Student No.],RTRIM(Student_name)[Student Names] from Student where ScholarNo like '" + textBoxX1.Text + "%' OR Student_name Like '" + textBoxX1.Text + "%'  order by ID DESC", con);
                SqlDataAdapter myDA = new SqlDataAdapter(cmd);
                DataSet myDataSet = new DataSet();
                myDA.Fill(myDataSet, "Student");
                dataGridView1.DataSource = myDataSet.Tables["Student"].DefaultView;
