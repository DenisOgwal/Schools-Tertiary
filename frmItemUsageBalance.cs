...
{
    public partial class frmItemUsageBalance : Form
    {
        ...
        private void button5_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                con.Open();
                cmd = new SqlCommand("select RTrim(UsageID)[Usage ID], RTRIM(year)[Session],RTRIM(term)[Semester],RTRIM(product)[Product],RTRIM(quantity)[Quantity],RTRIM(units)[Units],RTRIM(quality)[Quality],RTRIM(usagedate)[Date of Use],(balance)[Balance] from ItemUsage where product='" + Course.Text + "' order by usagedate", con);
        ...
        private void ScholarNo_SelectedIndexChanged(object sender, EventArgs e)
        {
            try
            {
                con = new SqlConnection(cs.DBConn);
                con.Open();
                cmd = new SqlCommand("select RTrim(UsageID)[Usage ID], RTRIM(year)[Session],RTRIM(term)[Semester],RTRIM(product)[Product],RTRIM(quantity)[Quantity],RTRIM(units)[Units],RTRIM(quality)[Quality],RTRIM(usagedate)[Date of Use],(balance)[Balance] from ItemUsage where UsageID= '" + ScholarNo.Text + "' order by usagedate", con);
                SqlDataAdapter myDA = new SqlDataAdapter(cmd);
                DataSet myDataSet = new DataSet();
                myDA.Fill(myDataSet, "ItemUsage");
        ...
        private void button10_Click_1(object sender, EventArgs e)
        {
            ...
            try
            {
                con = new SqlConnection(cs.DBConn);
                con.Open();
                cmd = new SqlCommand("select RTrim(UsageID)[Usage ID], RTRIM(year)[Session],RTRIM(term)[Semester],RTRIM(product)[Product],RTRIM(quantity)[Quantity],RTRIM(units)[Units],RTRIM(usagedate)[Date of Use],(balance)[Balance] from ItemUsage where usagedate between @date1 and @date2 order by usagedate", con);
                cmd.Parameters.Add("@date1", SqlDbType.DateTime, 30, "usagedate").Value = PaymentDateFrom.Value.Date;
                cmd.Parameters.Add("@date2", SqlDbType.DateTime, 30, "usagedate").Value = PaymentDateTo.Value.Date;
                SqlDataAdapter myDA = new SqlDataAdapter(cmd);
                DataSet myDataSet = new DataSet();
