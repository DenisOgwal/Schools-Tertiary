    ...
    public partial class frmSalaryPaymentRecord : Form
    {
        ...
        private void cmbStaffName_SelectedIndexChanged(object sender, EventArgs e)
        {
            try
            {
                ...
                con.Open();
                cmd = new SqlCommand("select RTrim(PaymentID)[Payment ID], RTRIM(Employee.StaffID)[Staff ID], RTRIM(Employee.StaffName)[Staff Name], RTRIM(EmployeePayment.BasicSalary1)[Basic Salary], RTRIM(PaymentDate)[Payment Date],RTRIM(ModeOfPayment)[Payment Mode], RTRIM(PaymentModeDetails)[Payment Mode Details], RTRIM(Deduction)[Deduction],RTRIM(TotalPaid)[Total Paid],RTRIM(DueFees)[Due Fees],RTRIM(Months)[Months],RTRIM(Year)[Session],RTRIM(Term)[Semester] from EmployeePayment,Employee where EmployeePayment.StaffID=Employee.StaffID and StaffName= '" + cmbStaffName.Text + "' order by ID", con);
