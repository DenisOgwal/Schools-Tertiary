    ...
    public partial class frmSalaryPaymentRecord1 : Form
    {
        ...
        private void cmbStaffName_SelectedIndexChanged(object sender, EventArgs e)
        {
            try
            {
                ...
                con.Open();
                cmd = new SqlCommand("select RTrim(PaymentID)[Payment ID], RTRIM(StaffID)[Staff ID],RTRIM(StaffName)[Staff Name], RTRIM(EmployeePayment.BasicSalary)[Basic Salary], RTRIM(PaymentDate)[Payment Date],RTRIM(ModeOfPayment)[Payment Mode], RTRIM(PaymentModeDetails)[Payment Mode Details], RTRIM(Deduction)[Deduction],RTRIM(TotalPaid)[Total Paid],RTRIM(DueFees)[Due Fees],RTRIM(Months)[Months],RTRIM(Year)[Session],RTRIM(Term)[Semester] from EmployeePayment where  StaffName= '" + cmbStaffName.Text + "' order by PaymentDate", con);
        ...
        private void DGV_RowHeaderMouseClick(object sender, DataGridViewCellMouseEventArgs e)
        {
            try
            {
                ...
                frm.session.Text = dr.Cells[11].Value.ToString();
